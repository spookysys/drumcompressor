...
function main(infile, outfile, checkdir, block_size, adpcm_bits)
    ...
    size2 = length(bass_sig_norm) * 2./8; % encoded bass size in bytes (one sample in this array is one block, and 2 bits per block)
    size = size1 + size2;
    ...
    % mix and output
    padding = zeros(length(data_in)/block_size - length(bass_recon), 1);
